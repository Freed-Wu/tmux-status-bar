...
def get_document(
    ...
    r"""Get document. ``builtin`` will use builtin make.json. ``cache``
    ...
    distribution's make doesn't contain textinfo. So we use ``builtin`` as
    default.
    ...
    if method == "builtin":
        ...
        if not os.path.exists(user_cache_dir("make.json")):
            document = init_document()
            with open(user_cache_dir("make.json"), "w") as f:
                json.dump(document, f)
        else:
            with open(user_cache_dir("make.json"), "r") as f:
                document = json.load(f)
    else:
        from .api import init_document
